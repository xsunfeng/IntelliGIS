...
namespace CAGA.Map
{
  ...
  /// </summary>
  public class MapAndLayoutSynchronizer
  {
    ...
    /// default constructor
    /// </summary>
    public MapAndLayoutSynchronizer()
    {
    ...
    public MapAndLayoutSynchronizer(IMapControl3 mapControl, IPageLayoutControl2 pageLayoutControl)
      : this()
    {
    ...
    /// </summary>
    /// <param name="syncExtent">true if synchronizing map view extent</param>
    public void ActivatePageLayout(bool syncExtent = false)
    {
      try
      {
        if (m_pageLayoutControl == null || m_mapControl == null)
        ...
        IEnvelope extent = m_mapControl.Extent;
    ...
    /// Sync map extent
    /// </summary>
    /// <param name="fromMapView">true if sync from MapControl to LayoutControl</param>
    public void SyncMapExtent(bool fromMapView=true)
    {
        if (m_pageLayoutControl == null || m_mapControl == null)
            throw new Exception("ControlsSynchronizer::BindControls:\r\nEither MapControl or PageLayoutControl are not initialized!");
        if (fromMapView == true)
        {
            this.m_pageLayoutControl.ActiveView.Extent = this.m_mapControl.Extent;
            this.m_pageLayoutControl.ActiveView.Refresh();
        }
        else
        {
            this.m_mapControl.Extent = this.m_pageLayoutControl.ActiveView.Extent;
